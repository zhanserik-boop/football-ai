...
def build_event_study(df, pre_window=PRE_WINDOW, post_windows=POST_WINDOWS):
    ...
    events = []
    change_rows = data.index[pd.to_numeric(data["coach_change_flag"], errors="coerce").fillna(0).astype(int) == 1]
    ...
    for idx in change_rows:
        ...
        for n in post_windows:
            ...
            if len(post) < n:
                continue
            # Do not cross into another coach spell.
            ...
            for metric in METRICS:
                ...
                rec[f"delta_{metric}"] = post_avg - pre_avg if pd.notna(pre_avg) and pd.notna(post_avg) else np.nan
            events.append(rec)
...
def run(input_file=INPUT_FILE, events_output=EVENTS_OUTPUT, summary_output=SUMMARY_OUTPUT):
    ...
    print("Confirmed coach changes in source:", int(pd.to_numeric(df["coach_change_flag"], errors="coerce").fillna(0).sum()))
